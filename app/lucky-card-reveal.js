...
const luckyCards = [
  {
    ...
    symbols: [{ icon: '🍀', label: 'Four-leaf clover', size: '5.1rem' }],
    accent: '#86efac',
    titleColor: '#dcfce7',
  ...
  },
  {
    ...
    symbols: [{ icon: '🂡', label: 'Ace of Spades playing card', size: '5.4rem' }],
    accent: '#facc15',
    titleColor: '#fef3c7',
  ...
  },
  {
    ...
    symbols: [
      { icon: '🃏', label: 'Joker', size: '2.55rem' },
      { icon: '∩', label: 'Lucky horseshoe', size: '2.75rem' },
      { icon: '🐇', label: "Rabbit's foot", size: '2.45rem' },
    ],
    accent: '#f0abfc',
    titleColor: '#fae8ff',
        ...
        >
          ...
          <span style={{ position: 'absolute', inset: 0, display: 'grid', gridTemplateRows: 'auto 1fr auto', gap: '0.45rem', alignItems: 'center', borderRadius: 28, backfaceVisibility: 'hidden', transform: 'rotateY(180deg)', background: cardFace.faceBackground, color: '#fff7d6', border: '3px solid #facc15', boxShadow: `inset 0 0 34px rgba(250, 204, 21, 0.3), 0 0 34px ${cardFace.accent}66, 0 18px 34px rgba(0, 0, 0, 0.3)`, overflow: 'hidden', padding: '0.95rem 0.78rem' }}>
            ...
            <span aria-label={`${cardFace.name} artwork`} style={{ position: 'relative', zIndex: 1, display: 'grid', placeItems: 'center', minHeight: 122, borderRadius: 22, background: cardFace.artworkFrame, border: `1px solid ${cardFace.accent}88`, boxShadow: `inset 0 0 28px rgba(255, 247, 214, 0.16), 0 0 24px ${cardFace.accent}55`, opacity: isRevealed ? 1 : 0, animation: isRevealed ? 'lucky-artwork-rise 0.52s ease 0.2s both' : 'none' }}>
              ...
              <span style={{ position: 'relative', zIndex: 1, display: 'flex', gap: '0.32rem', alignItems: 'center', justifyContent: 'center', flexWrap: 'wrap' }}>
                {cardFace.symbols.map((symbol, index) => (
                  <span key={`${symbol.label}-${index}`} title={symbol.label} style={{ display: 'inline-grid', placeItems: 'center', width: cardFace.symbols.length > 1 ? 54 : 104, height: cardFace.symbols.length > 1 ? 54 : 104, borderRadius: cardFace.name === 'Royal Luck Card' ? 14 : '50%', background: 'radial-gradient(circle, rgba(255, 247, 214, 0.92), rgba(250, 204, 21, 0.2), transparent 72%)', color: cardFace.name === 'Royal Luck Card' ? '#020617' : undefined, fontSize: symbol.size, lineHeight: 1, fontWeight: 900, textShadow: '0 0 18px rgba(255, 247, 214, 0.7)' }}>
                    {symbol.icon}
                  </span>
                ))}
              </span>
