...
export default function DailyMeterWidget() {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      ctx.fillStyle = '#fff8df'; // Base cream color
      ...
      stars.forEach((star) => {
        ...
        // Using globalAlpha instead of string interpolation (rgba) prevents thousands
        // of string allocations and GC pauses per second.
        ...
        {!isRevealed || isGenerating ? (
          <div className="relative w-[130px] h-[130px] flex items-center justify-center shrink-0 z-10 mt-1">
            <svg
              ...
              style={{ filter: `drop-shadow(0 5px 15px rgba(0, 0, 0, 0.85)) drop-shadow(0 0 ${isRevealed ? '30px' : '15px'} ${isRevealed ? '#e8ba52' : tierGlow})` }}
            >
              <defs>
