  ...
  return (
    <div className="lucky-map-shell cosmic-aurora-background bg-[#030712]" style={pageStyle}>
      ...
      <div style={{ position: 'relative', maxWidth: 1180, margin: '0 auto' }}>
        ...
        {isStoryFormOpen ? <div className="story-modal-backdrop" onMouseDown={() => setIsStoryFormOpen(false)}><section role="dialog" aria-modal="true" aria-labelledby="story-form-title" className="story-modal" onMouseDown={(event) => event.stopPropagation()}><button type="button" aria-label="Close story form" onClick={() => setIsStoryFormOpen(false)} autoFocus>×</button><h2 id="story-form-title">Share your lucky story</h2><form action="/api/lucky-stories" method="post"><input name="website" tabIndex="-1" autoComplete="off" style={{ display: 'none' }} /><label><span className="inline-flex items-center gap-1">Name<span aria-hidden="true" className="text-red-500">*</span></span><input name="name" required maxLength="40" /></label><label>Province or territory<input name="location" maxLength="80" /></label><label><span className="inline-flex items-center gap-1">Your story<span aria-hidden="true" className="text-red-500">*</span></span><textarea name="story" required minLength="20" maxLength="1500" rows="5" /></label><TurnstileField siteKey={process.env.NEXT_PUBLIC_TURNSTILE_SITE_KEY} submitButtonId="lucky-story-submit" /><button id="lucky-story-submit" type="submit">Submit story</button></form></section></div> : null}
      </div>
    </div>
  );
}
