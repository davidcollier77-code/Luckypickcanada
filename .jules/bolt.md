...
## 2024-05-19 - [HTML5 Canvas globalAlpha + Radial Gradient Regression]
**Learning:** When optimizing HTML5 Canvas `createRadialGradient` by converting dynamic `rgba(R,G,B,A)` string interpolations to static hex colors and using `ctx.globalAlpha`, it is critical **not** to change the final transparent gradient color stop (e.g., `glow.addColorStop(1, 'rgba(0,0,0,0)')`) to an opaque hex code like `#000000`. Doing so, even when `globalAlpha` is applied, results in a semi-transparent black stop rather than true transparency. If combined with bounding-box `fillRect` optimization, this will render ugly, overlapping semi-transparent black squares instead of soft, circular glows.
**Action:** Always strictly preserve the `rgba(0,0,0,0)` (or `#00000000` if supported) stop for the transparent outer edges of gradients, even while converting the inner solid color stops to static Hex codes.
